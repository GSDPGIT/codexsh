...
def send_tg_msg(msg: str) -> None:
    try: urllib.request.urlopen(TG_API + urllib.parse.quote(msg), timeout=5)
...
def cleanup_journal() -> None:
    try: subprocess.run(["journalctl", f"--vacuum-size={JOURNAL_VACUUM_SIZE}"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
    except Exception: pass
...
def fire_batch_task() -> bool:
    ...
    req = urllib.request.Request(f"{HOST}/api/registration/batch", data=json.dumps(PAYLOAD).encode("utf-8"), headers=HEADERS, method="POST")
    try:
            ...
            for key in ("success", "ok"):
                if key in data:
                    if bool(data[key]):
                        ...
                        return True
                    return False
            if "code" in data and str(data["code"]) not in {"0", "200", "success", "ok"}: return False
        return True
    except Exception:
        if any(word in body_text.lower() for word in ["error", "failed", "exception"]): return False
        return True
...
def monitor() -> int:
    ...
    process = subprocess.Popen(["journalctl", "-u", "codex-console", "-f", "-n", "0"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setsid)
    ...
    try:
        while True:
            ...
            if not line:
                if current_time - last_activity_time > IDLE_TIMEOUT_SECONDS: return 1
                time.sleep(POLL_INTERVAL_SECONDS)
                continue

            last_activity_time = current_time

            if "注册任务完成" in line: success_count += 1
            elif any(k in line for k in ["放弃注册", "注册失败", "获取不到验证码"]): fail_count += 1

            if success_count + fail_count >= target:
                print(f"\n🎉 本轮达成！成功 {success_count}，失败 {fail_count}。")
                send_tg_msg(f"🎉 本轮 {target} 个任务结束！\n成功: {success_count}\n失败: {fail_count}\n即将进入休眠。")
                return 0
    finally:
