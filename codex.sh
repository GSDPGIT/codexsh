#!/bin/bash
# ==========================================
# Codex Console 自动化管理面板 v5.2 (修复版)
# 修复内容：
#   1. deploy_env() 中创建 systemd service 文件
#   2. start_engine() 添加前置检查
#   3. generate_daemon() 修复 heredoc 变量引用
#   4. 全部使用 LF 行尾
#   5. 密码特殊字符安全处理
# ==========================================
GREEN='\033[0;32m'
...
print_header() {
    clear
    echo -e "${GREEN}====================================================${NC}"
    echo -e "${YELLOW}   Codex Console 自动化管理面板 v5.2 (修复版)        ${NC}"
    echo -e "${GREEN}====================================================${NC}"
}
...
check_or_ask_password() {
    if [ ! -f "$ENV_FILE" ]; then
        ...
        # 使用 printf %q 安全转义密码中的特殊字符
        escaped_pwd=$(printf '%q' "$input_pwd")
        echo "export CODEX_PASSWORD=$escaped_pwd" > "$ENV_FILE"
        chmod 600 "$ENV_FILE"
        sync_passwords_now
    fi
    source "$ENV_FILE"
...
generate_daemon() {
    # 注意：heredoc 内所有 $ 变量分为两类：
    #   - 立即展开的（来自 codex.sh 的配置变量）：直接写 $VAR
    #   - 延迟展开的（运行时在 daemon.sh 内使用）：写 \$VAR
    # 此处我们把所有配置在顶部赋值后，循环体内全部使用延迟引用
...
start_engine() {
    ...
    # ✅ 修复：前置检查，确保环境已部署
    if [ ! -f "$VENV_PY" ]; then
        echo -e "${RED}❌ 虚拟环境不存在 ($VENV_PY)！${NC}"
        echo -e "${YELLOW}👉 请先执行选项 1【全能部署】安装环境。${NC}"
        return 1
    fi
...
main_menu() {
    while true; do
        ...
        case "$choice" in
            ...
            3) systemctl stop codex-console 2>/dev/null; pkill -9 -f daemon.sh; pkill -9 -f auto_run.py; echo "已物理超度"; read -p "按回车继续..." ;;
